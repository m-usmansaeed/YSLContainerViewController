...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        CGFloat fillkYSLScrollMenuViewWidth = 0.0;
       
        if ((itemTitleArray.count * kYSLScrollMenuViewWidth) > [UIScreen mainScreen].bounds.size.width) {
            fillkYSLScrollMenuViewWidth = kYSLScrollMenuViewWidth;
        }else{
            fillkYSLScrollMenuViewWidth = (([UIScreen mainScreen].bounds.size.width) / self.itemViewArray.count) - (kYSLScrollMenuViewMargin * self.itemViewArray.count) + kYSLScrollMenuViewMargin;
        }
        
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame =  CGRectMake(0, 0, fillkYSLScrollMenuViewWidth, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake(10, _scrollView.frame.size.height - kYSLIndicatorHeight, fillkYSLScrollMenuViewWidth, kYSLIndicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if ((self.itemViewArray.count * kYSLScrollMenuViewWidth) > [UIScreen mainScreen].bounds.size.width) {
        fillkYSLScrollMenuViewWidth = kYSLScrollMenuViewWidth;
    }else{
        fillkYSLScrollMenuViewWidth = (([UIScreen mainScreen].bounds.size.width) / self.itemViewArray.count) - (kYSLScrollMenuViewMargin * self.itemViewArray.count) + kYSLScrollMenuViewMargin;
    }
    ...
    if (isNextItem) {
        indicatorX = ((kYSLScrollMenuViewMargin + fillkYSLScrollMenuViewWidth) * ratio ) + (toIndex * fillkYSLScrollMenuViewWidth) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    } else {
        indicatorX =  ((kYSLScrollMenuViewMargin + fillkYSLScrollMenuViewWidth) * (1 - ratio) ) + (toIndex * fillkYSLScrollMenuViewWidth) + ((toIndex + 1) * kYSLScrollMenuViewMargin);
    }
    
    if (indicatorX < kYSLScrollMenuViewMargin || indicatorX > self.scrollView.contentSize.width - (kYSLScrollMenuViewMargin + fillkYSLScrollMenuViewWidth)) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - kYSLIndicatorHeight, fillkYSLScrollMenuViewWidth, kYSLIndicatorHeight);
...
- (void)layoutSubviews
{
    ...
    for (NSUInteger i = 0; i < self.itemViewArray.count; i++) {
        CGFloat width = fillkYSLScrollMenuViewWidth;
