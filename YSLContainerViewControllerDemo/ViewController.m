...
@property (strong, nonatomic) YSLContainerViewController *containerVC;
@end
...
//    sampleVC3.title = @"Setting";
    ...
    self.containerVC = [[YSLContainerViewController alloc]
                        initWithControllers:@[playListVC,artistVC]
                        topBarHeight:statusHeight + navigationHeight
                        parentViewController:self];

    self.containerVC.delegate = self;
    self.containerVC.menuItemFont = [UIFont fontWithName:@"Futura-Medium" size:16];
    self.containerVC.menuItemTitleColor = [UIColor lightGrayColor];
    self.containerVC.menuItemSelectedTitleColor = [UIColor redColor];
    self.containerVC.menuBackGroudColor = [UIColor whiteColor];
    self.containerVC.menuIndicatorColor = [UIColor redColor];
...
//    dispatch_async(dispatch_get_main_queue(), ^{
        [self.containerVC scrollMenuViewSelectedIndex:1];
//    });
